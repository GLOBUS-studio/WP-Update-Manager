...
class Complete_Updates_Manager_Admin {
	...
	public function initialize() {
		...
		add_filter( 'plugin_action_links_' . WUM_PLUGIN_BASENAME, array( $this, 'add_plugin_action_links' ) );

		// Add first activation notice
		add_action( 'admin_notices', array( $this, 'show_first_activation_notice' ) );
	}
	...
	public function enqueue_admin_styles() {
		wp_add_inline_style(
			'admin-bar',
			'.wp-admin-bar-wum-notice { background-color: rgba(255, 37, 37, 0.4) !important; } ' .
			'.wp-admin-bar-wum-notice .dashicons { font-family: dashicons !important; padding-top: 7px; } ' .
			'.plugins .wum-settings { color: #8b0d0d; font-weight: bold; }'
		);

		wp_enqueue_script(
			'wum-admin',
			WUM_PLUGIN_URL . 'assets/admin.js',
			array( 'jquery' ),
			WUM_VERSION,
			true
		);
