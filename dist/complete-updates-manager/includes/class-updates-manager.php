...
class Complete_Updates_Manager {
	...
	public function initialize() {
		...
		add_action( 'admin_init', array( $this, 'disable_updates_admin_init' ) );

		// Filter transients before they are set
		// to prevent update checks
		add_filter( 'pre_transient_update_themes', array( $this, 'override_version_check' ) );
		add_filter( 'pre_site_transient_update_themes', array( $this, 'override_version_check' ) );
		add_filter( 'pre_transient_update_plugins', array( $this, 'override_version_check' ) );
		add_filter( 'pre_site_transient_update_plugins', array( $this, 'override_version_check' ) );
		add_filter( 'pre_transient_update_core', array( $this, 'override_version_check' ) );
		add_filter( 'pre_site_transient_update_core', array( $this, 'override_version_check' ) );

		// Priority 21 ensures our filter runs after standard WordPress checks
		add_action( 'pre_set_site_transient_update_plugins', array( $this, 'override_version_check' ), 21 );
		add_action( 'pre_set_site_transient_update_themes', array( $this, 'override_version_check' ), 21 );

		// Filter cron events and block HTTP requests to update API
		add_action( 'schedule_event', array( $this, 'filter_cron_events' ) );
		add_filter( 'pre_http_request', array( $this, 'block_update_requests' ), 10, 3 );
	...
	public function disable_updates_admin_init() {
		...
		add_filter( 'site_status_tests', array( $this, 'remove_update_health_checks' ) );
	...
	public function override_version_check( $transient ) {
		...
		if ( $return_empty_transient_flag ) {
			...
			if ( strpos( $current_filter, 'update_core' ) !== false ) {
				...
				$empty_transient_obj->updates         = array();
				$empty_transient_obj->response        = 'latest'; // Standard for no core updates
			} else { // Plugins or Themes
				$empty_transient_obj->response     = array();
				$empty_transient_obj->translations = array();
			}
	 ...
	 *
	 * Uses the user_has_cap filter to remove update capabilities
	 * for additional protection against updates.
	 *
	 * @return void
	 */
	private function remove_update_capabilities() {
		add_filter( 'user_has_cap', array( $this, 'filter_user_update_capabilities' ), 10, 4 );
	}

	/**
	 * Filter user capabilities to remove update-related caps
	 *
	 ...
	 */
	public function filter_user_update_capabilities( $allcaps, $caps, $args, $user ) {
	...
	public function hide_update_ui() {
		...
		// Enqueue CSS to hide update counters
		add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_admin_assets' ) );
	}

	/**
	 * Enqueue admin CSS to hide update UI elements
	 *
	 * @since  1.0.2
	 * @return void
	 */
	public function enqueue_admin_assets() {
		wp_enqueue_style(
			'wum-admin',
			WUM_PLUGIN_URL . 'assets/admin.css',
			array(),
			WUM_VERSION
		);
	}
