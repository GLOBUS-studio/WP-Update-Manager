...
class Complete_Updates_Manager_Settings {
	...
	public function initialize() {
		...
		if ( $this->is_security_monitoring_enabled() ) {
			add_action( 'admin_init', array( $this, 'check_security_updates' ) );
			add_action( 'admin_notices', array( $this, 'display_security_notices' ) );
		}
	}
	...
	public function sanitize_settings( $input ) {
		...
		$checkboxes = array( 'disable_core_updates', 'disable_plugin_updates', 'disable_theme_updates', 'monitor_security_updates', 'disable_plugins_api_filter' );
			...
			<table class="widefat fixed" style="max-width:900px;">
				...
				<tr>
					...
					<td style="text-align:center;">
						...
						wum_render_freeze_version_field( 'core', '', $core_version, $frozen_core, false );
						?>
					</td>                    <td>
						<?php
				...
				<tr>
					...
					<td style="text-align:center;">
						...
						wum_render_freeze_version_field( 'plugin', $file, $data['Version'], $frozen_plugin, false );
						?>
					</td>                    <td>
						<?php
				...
				<tr>
					...
					<td style="text-align:center;">
						...
						wum_render_freeze_version_field( 'theme', $slug, $theme->get( 'Version' ), $frozen_theme, false );
						?>
					</td>                    <td>
						<?php
	...
	public function maybe_save_version_freeze() {
		...
		if ( isset( $_POST['wum_version_freeze_nonce'] ) && wp_verify_nonce( $_POST['wum_version_freeze_nonce'], 'wum_version_freeze_action' ) ) {
			$freeze = isset( $_POST['wum_version_freeze'] ) ? $_POST['wum_version_freeze'] : array();
			$clean  = array();
			...
			if ( ! empty( $freeze['plugin'] ) && is_array( $freeze['plugin'] ) ) {
				foreach ( $freeze['plugin'] as $file => $ver ) {
					if ( ! empty( $ver ) ) {
						$clean['plugin'][ $file ] = sanitize_text_field( $ver );
					}
				}
			}
			if ( ! empty( $freeze['theme'] ) && is_array( $freeze['theme'] ) ) {
				foreach ( $freeze['theme'] as $slug => $ver ) {
					if ( ! empty( $ver ) ) {
						$clean['theme'][ $slug ] = sanitize_text_field( $ver );
					}
				}
			}
	...
	public function display_security_notices() {
		$security_issues = get_option( 'wum_security_issues', array() );
